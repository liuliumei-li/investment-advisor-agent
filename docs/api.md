...
大盘研判为 UC-02 主流程:创建会话 → SSE 流式咨询 → 查看历史/建议详情。已开放 `market`(大盘研判)与 `industry`(行业/概念板块分析)两个场景(其余待 US-08~11 开放,当前返回 40001 提示)。研判链路:实时行情/板块/快讯/研报取数(免费公开源 + SkillHub 占位)→ LLM 生成 → 幻觉校验 gate → 合规审核 gate → 四要素落库,全程经数据源白名单与引用溯源(BR-DAT-02/04/05、BR-CMP-01/02)。
...
`scenario` 取值:`market`(大盘研判)/ `industry`(行业与概念板块分析);其余取值 40001「尚未开放」。
