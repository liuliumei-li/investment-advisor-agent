...
## 12. 咨询会话接口(US-06 大盘研判、US-07 板块分析)

大盘研判为 UC-02 主流程:创建会话 → SSE 流式咨询 → 查看历史/建议详情。已开放 `market`(大盘研判)、`industry`(行业/概念板块分析)、`stock`(个股分析,两段式:先提取股票代码再取数分析;个股行情来自腾讯、财务指标来自东财 F10,单源故障降级标注、双源全故障拒答)三个场景(其余待 US-09~11 开放,当前返回 40001 提示)。研判链路:实时行情/板块/快讯/研报/财务取数(免费公开源 + SkillHub 占位)→ LLM 生成 → 幻觉校验 gate → 合规审核 gate → 四要素落库,全程经数据源白名单与引用溯源(BR-DAT-02/04/05、BR-CMP-01/02)。个股分析的 risk_tips 强制追加「基于公开数据,不含未公开信息;财务数据来自定期报告,存在披露滞后」。
...
`scenario` 取值:`market`(大盘研判)/ `industry`(行业与概念板块分析)/ `stock`(个股分析);其余取值 40001「尚未开放」。
